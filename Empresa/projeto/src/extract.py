...
def OpenFile():
    ...
    local_arquivo = Label(inter.tb1 , text=filepath,font=("Arial",10)) 
    local_arquivo.place(x=30,y=55)
...
def Openfile_2():
    global filepath
    filepath = filedialog.askopenfilename()
    local_arquivo = Label(inter.tb2 , text=filepath,font=("Arial",10)) 
    local_arquivo.place(x=30,y=55)
...
def pega_debitos(df,index,row):
    ...
    try:
        ...
        df['DEBITOS'][index] = 'Não foi possivel pesquisar.'
        df['DEBITOS TOTAIS'][index] = 'Não foi possivel pesquisar.'
        df.to_excel(f"{inter.nome_arquivo2}.xlsx", index=False)
